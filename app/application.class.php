...
class wcCartGifts{
  private $pluginInfo;
	...
	public function locate_template( $args ){
		...
		$template = locate_template( array(
			'wc-cart-gifts' . DIRECTORY_SEPARATOR . $template_name,
		) );
  ...
  public function camel2dashed( $class_name ) {
		...
		if( ! class_exists( 'acf' ) ) {
			add_action('admin_notices', function(){ ?>
				<div class="notice notice-error">
					<p><?php _e('WC Cart Gifts depends on Advanced Custom Fields PRO, please activate it.', 'wc-cart-gifts'); ?></p>
				</div>
			<?php });
			return false;
		}
    ...
    spl_autoload_register(function ($class_name) {
      if( strpos( $class_name, 'wcCartGifts') === 0 ){
				$classPath = WC_CART_GIFTS_APP_PATH;
				if( strpos( $class_name, 'wcCartGifts\config') === 0 ){
					$classPath = WC_CART_GIFTS_DIR_PATH;
				}
        $class_name = str_replace( '\\', DIRECTORY_SEPARATOR, $class_name );
				$class_name = str_replace( 'wcCartGifts' . DIRECTORY_SEPARATOR, $classPath, $class_name );
  ...
  public function load_classes(){
    $this->cart_gift = new wcCartGifts\model\cartGift;

    $this->cartGifts = new wcCartGifts\controller\cartGifts;
    $this->cart = new wcCartGifts\controller\cart;
    $this->order = new wcCartGifts\controller\order;
  }
  ...
  public function enqueue_scripts(){
	...
	public function admin_enqueue_scripts(){
		// wp_enqueue_style( 'wc-cart-gifts-style', WC_CART_GIFTS_ASSETS_URL . '/css/backend.css', array(), '0.1', 'all' );

		// wp_enqueue_script( 'wc-cart-gifts-script', WC_CART_GIFTS_ASSETS_URL . '/js/backend.js', array('jquery'), '0.1', true );
	}
  
	public function load_plugin_textdomain(){
		load_plugin_textdomain( 'wc-cart-gifts', false, basename( dirname( WC_CART_GIFTS_PLUGIN_FILE ) ) . '/languages' ); 
	}
	...
	public function getPluginInfo( $info = '' ){
		if( !$this->pluginInfo ){
			$this->pluginInfo = get_plugin_data( WC_CART_GIFTS_DIR_PATH . 'wc-cart-gifts.php' );
		}
		if( $info ){
